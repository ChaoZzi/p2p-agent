-- ============================================================================
-- p2p-agent 专用 PostgreSQL 角色与库（P0-02 §0 前置，一次性执行）
-- ----------------------------------------------------------------------------
-- 用法（在你自己的终端里跑，密码由你当场输入，不会写进仓库、不会进 shell 历史）：
--     "D:\software\postgres\bin\psql.exe" -U postgres -h 127.0.0.1 -f scripts/pg-init.sql
--
-- 之后把密码填进【本地未提交】的配置：
...
\echo '=== 建立角色 p2p_agent（最小权限：不做超级用户、不建库建角色） ==='
...
-- 用 psql 变量设密码（避免明文出现在文件/历史里）
ALTER ROLE p2p_agent PASSWORD :'p2p_pw';

\echo '=== 建立库 p2p_agent（owner=p2p_agent，UTF8） ==='
...
\echo '=== 连接测试（应成功连上，无权建超级用户） ==='
\connect p2p_agent
SELECT current_user, current_database(), version();

-- 说明：审计表的 append-only 权限（REVOKE UPDATE/DELETE ON audit_log）在 P0-02 §0
-- 建表时由 schema-postgres.sql 一并处理，这里只管角色与库。

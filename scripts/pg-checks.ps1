...
Write-Output ""
Write-Output "### append-only 探针 1/4：INSERT 允许（事务内插入后回滚，不留数据）"
...
Write-Output ""
Write-Output "### append-only 探针 2/4：UPDATE 必须被拒绝（期望 ERROR: permission denied）"
& $psql @base -c "update audit_log set actor='attacker' where false;"
Write-Output ""
Write-Output "### append-only 探针 3/4：DELETE 必须被拒绝（期望 ERROR: permission denied）"
& $psql @base -c "delete from audit_log where false;"
Write-Output ""
Write-Output "### append-only 探针 4/4：TRUNCATE 也必须被拒绝（事务内执行后回滚，不留副作用）"
& $psql @base -c "begin; truncate audit_log; rollback;"
